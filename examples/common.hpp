...
void print_frequency_comparison(
    const std::string &title, const std::vector<uint64_t> &items, const std::map<uint64_t, uint64_t> &true_freqs, const std::vector<std::string> &sketch_names,
    const SketchTypes &...sketches);

// Helper for the print_frequency_comparison variadic template
template <typename SketchType, typename... RestSketchTypes>
void print_frequency_comparison_impl(
    uint64_t item, const std::map<uint64_t, uint64_t> &true_freqs, const std::vector<std::string> &sketch_names, size_t idx, const SketchType &sketch,
    const RestSketchTypes &...rest);
