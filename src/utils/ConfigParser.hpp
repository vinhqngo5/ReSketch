...
class Status {
  public:
    ...
    static Status NotFound(const std::string &message1, const std::string &message2 = "") { return Status(kNotFound, message1, message2); }

    static Status InvalidArgument(const std::string &message1, const std::string &message2 = "") { return Status(kInvalidArgument, message1, message2); }

    static Status IOError(const std::string &message1, const std::string &message2 = "") { return Status(kIOError, message1, message2); }
  ...
  private:
    ...
    enum Code { kOK = 0, kNotFound = 1, kDeleteOrder = 2, kInvalidArgument = 3, kIOError = 4, kDone = 5, kMultipartRequired = 6 };
};
...
class Parameter {
  public:
    ...
    uint64_t GetMultiplier(std::string str) {
        ...
        std::smatch matches;
        if (!std::regex_search(str, matches, regex_number) || matches.size() != 3) { return 1; }
...
class IntParameter : public Parameter {
  public:
    int *ptr;
    IntParameter(const std::string &name_in, const std::string default_in, int *ptr_in, bool mandatory_in, const std::string &description_in) {
        name = name_in;
        is_mandatory = mandatory_in;
        description = description_in;
        default_value = default_in;
    ...
    virtual Status Parse(const std::string &config, const std::string &value, const std::string &filepath, int line_number) {
        int num_scanned = sscanf(value.c_str(), "%u", ptr);
        if (num_scanned != 1) {
            std::string str_line_number = "Invalid value for 32-bit integer parameter [" + config + "] in file [" + filepath + "] on line " + std::to_string(line_number);
            return Status::IOError("ConfigParser", str_line_number);
        }
        uint64_t multiplier = GetMultiplier(value);
        if (multiplier == 0) {
...
class FloatParameter : public Parameter {
  public:
    float *ptr;
    FloatParameter(const std::string &name_in, const std::string &default_in, float *ptr_in, bool mandatory_in, const std::string &description_in) {
        name = name_in;
        is_mandatory = mandatory_in;
        description = description_in;
        default_value = default_in;
    ...
    virtual Status Parse(const std::string &config, const std::string &value, const std::string &filepath, int line_number) {
        ...
        if (num_scanned != 1) {
            std::string str_line_number = "Invalid value for float-precision number parameter [" + config + "] in file [" + filepath + "] on line " + std::to_string(line_number);
            return Status::IOError("ConfigParser", str_line_number);
        }
        return Status::OK();
    }
...
class ConfigParser {
  public:
    ...
    void PrintAllMissingMandatoryParameters() {
        ...
        for (auto &name : mandatories_) { fprintf(stderr, "%s\n", name.c_str()); }
    }
    ...
    Status ParseFile(std::string filepath) {
        ...
        struct stat info;
        if (stat(filepath.c_str(), &info) != 0) { return Status::IOError("ConfigParser", "The file specified does not exists: " + filepath); }
