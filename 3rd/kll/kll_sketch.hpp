...
class kll_sketch {
  public:
     ...
     */
    template <typename TT, typename CC, typename AA> explicit kll_sketch(const kll_sketch<TT, CC, AA> &other, const C &comparator = C(), const A &allocator = A());

    /**
     * Updates this sketch with the given data item.
     * @param item from a stream of items
     */
    template <typename FwdT> void update(FwdT &&item);

    /**
     * Merges another sketch into this one.
     * @param other sketch to merge into this one
     */
    template <typename FwdSk> void merge(FwdSk &&other);
     ...
     */
    template <typename TT = T, typename std::enable_if<std::is_arithmetic<TT>::value, int>::type = 0> static size_t get_max_serialized_size_bytes(uint16_t k, uint64_t n);
     ...
     * @param sd instance of a SerDe
     */
    template <typename SerDe = serde<T>> void serialize(std::ostream &os, const SerDe &sd = SerDe()) const;
     ...
     */
    template <typename SerDe = serde<T>> vector_bytes serialize(unsigned header_size_bytes = 0, const SerDe &sd = SerDe()) const;
     ...
     */
    template <typename SerDe = serde<T>> static kll_sketch deserialize(std::istream &is, const SerDe &sd = SerDe(), const C &comparator = C(), const A &allocator = A());
  ...
  private:
    ...
    // for deserialization
    class items_deleter;
    kll_sketch(uint16_t k, uint16_t min_k, uint64_t n, uint8_t num_levels, vector_u32 &&levels, std::unique_ptr<T, items_deleter> items, uint32_t items_size,
    ...
    template <typename FwdSk> void populate_work_arrays(FwdSk &&other, T *workbuf, uint32_t *worklevels, uint8_t provisional_num_levels);
    ...
    template <typename TT = T, typename std::enable_if<std::is_floating_point<TT>::value, int>::type = 0> static inline bool check_update_item(TT item) {
        return !std::isnan(item);
    }

    template <typename TT = T, typename std::enable_if<!std::is_floating_point<TT>::value, int>::type = 0> static inline bool check_update_item(TT) { return true; }
...
template <typename T, typename C, typename A> class kll_sketch<T, C, A>::const_iterator {
  public:
