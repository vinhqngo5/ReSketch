...
kll_sketch<T, C, A>::kll_sketch(uint16_t k, const C &comparator, const A &allocator)
    : comparator_(comparator), allocator_(allocator), k_(k), m_(kll_constants::DEFAULT_M), min_k_(k), num_levels_(1), is_level_zero_sorted_(false), n_(0), levels_(2, 0, allocator),
      items_(nullptr), items_size_(k_), min_item_(), max_item_(), sorted_view_(nullptr) {
    if (k < kll_constants::MIN_K || k > kll_constants::MAX_K) {
        throw std::invalid_argument("K must be >= " + std::to_string(kll_constants::MIN_K) + " and <= " + std::to_string(kll_constants::MAX_K) + ": " + std::to_string(k));
    }
...
kll_sketch<T, C, A>::kll_sketch(const kll_sketch &other)
    : comparator_(other.comparator_), allocator_(other.allocator_), k_(other.k_), m_(other.m_), min_k_(other.min_k_), num_levels_(other.num_levels_),
      is_level_zero_sorted_(other.is_level_zero_sorted_), n_(other.n_), levels_(other.levels_), items_(nullptr), items_size_(other.items_size_), min_item_(other.min_item_),
      max_item_(other.max_item_), sorted_view_(nullptr) {
    items_ = allocator_.allocate(items_size_);
...
kll_sketch<T, C, A>::kll_sketch(kll_sketch &&other) noexcept
    : comparator_(std::move(other.comparator_)), allocator_(std::move(other.allocator_)), k_(other.k_), m_(other.m_), min_k_(other.min_k_), num_levels_(other.num_levels_),
      is_level_zero_sorted_(other.is_level_zero_sorted_), n_(other.n_), levels_(std::move(other.levels_)), items_(other.items_), items_size_(other.items_size_),
      min_item_(std::move(other.min_item_)), max_item_(std::move(other.max_item_)), sorted_view_(nullptr) {
    other.items_ = nullptr;
}

template <typename T, typename C, typename A> kll_sketch<T, C, A> &kll_sketch<T, C, A>::operator=(const kll_sketch &other) {
...
template <typename T, typename C, typename A> kll_sketch<T, C, A> &kll_sketch<T, C, A>::operator=(kll_sketch &&other) {
...
template <typename T, typename C, typename A> kll_sketch<T, C, A>::~kll_sketch() {
    if (items_ != nullptr) {
...
kll_sketch<T, C, A>::kll_sketch(const kll_sketch<TT, CC, AA> &other, const C &comparator, const A &allocator)
    : comparator_(comparator), allocator_(allocator), k_(other.k_), m_(other.m_), min_k_(other.min_k_), num_levels_(other.num_levels_),
      is_level_zero_sorted_(other.is_level_zero_sorted_), n_(other.n_), levels_(other.levels_, allocator_), items_(nullptr), items_size_(other.items_size_),
      min_item_(other.min_item_), max_item_(other.max_item_), sorted_view_(nullptr) {
    static_assert(std::is_constructible<T, TT>::value, "Type converting constructor requires new type to be constructible from existing type");
    items_ = allocator_.allocate(items_size_);
    for (auto i = levels_[0]; i < levels_[num_levels_]; ++i) new (&items_[i]) T(other.items_[i]);
    check_sorting();
}

template <typename T, typename C, typename A> template <typename FwdT> void kll_sketch<T, C, A>::update(FwdT &&item) {
...
template <typename T, typename C, typename A> void kll_sketch<T, C, A>::update_min_max(const T &item) {
    if (is_empty()) {
...
template <typename T, typename C, typename A> uint32_t kll_sketch<T, C, A>::internal_update() {
...
template <typename T, typename C, typename A> template <typename FwdSk> void kll_sketch<T, C, A>::merge(FwdSk &&other) {
    if (other.is_empty()) return;
    if (m_ != other.m_) { throw std::invalid_argument("incompatible M: " + std::to_string(m_) + " and " + std::to_string(other.m_)); }
    if (is_empty()) {
...
template <typename T, typename C, typename A> bool kll_sketch<T, C, A>::is_empty() const { return n_ == 0; }

template <typename T, typename C, typename A> uint16_t kll_sketch<T, C, A>::get_k() const { return k_; }

template <typename T, typename C, typename A> uint64_t kll_sketch<T, C, A>::get_n() const { return n_; }

template <typename T, typename C, typename A> uint32_t kll_sketch<T, C, A>::get_num_retained() const { return levels_[num_levels_] - levels_[0]; }

template <typename T, typename C, typename A> bool kll_sketch<T, C, A>::is_estimation_mode() const { return num_levels_ > 1; }

template <typename T, typename C, typename A> T kll_sketch<T, C, A>::get_min_item() const {
    if (is_empty()) throw std::runtime_error("operation is undefined for an empty sketch");
    return *min_item_;
}

template <typename T, typename C, typename A> T kll_sketch<T, C, A>::get_max_item() const {
    if (is_empty()) throw std::runtime_error("operation is undefined for an empty sketch");
    return *max_item_;
}

template <typename T, typename C, typename A> C kll_sketch<T, C, A>::get_comparator() const { return comparator_; }

template <typename T, typename C, typename A> A kll_sketch<T, C, A>::get_allocator() const { return allocator_; }

template <typename T, typename C, typename A> double kll_sketch<T, C, A>::get_rank(const T &item, bool inclusive) const {
    if (is_empty()) throw std::runtime_error("operation is undefined for an empty sketch");
    setup_sorted_view();
    return sorted_view_->get_rank(item, inclusive);
}

template <typename T, typename C, typename A> auto kll_sketch<T, C, A>::get_PMF(const T *split_points, uint32_t size, bool inclusive) const -> vector_double {
    if (is_empty()) throw std::runtime_error("operation is undefined for an empty sketch");
    setup_sorted_view();
    return sorted_view_->get_PMF(split_points, size, inclusive);
}

template <typename T, typename C, typename A> auto kll_sketch<T, C, A>::get_CDF(const T *split_points, uint32_t size, bool inclusive) const -> vector_double {
    if (is_empty()) throw std::runtime_error("operation is undefined for an empty sketch");
    setup_sorted_view();
    return sorted_view_->get_CDF(split_points, size, inclusive);
}

template <typename T, typename C, typename A> auto kll_sketch<T, C, A>::get_quantile(double rank, bool inclusive) const -> quantile_return_type {
    if (is_empty()) throw std::runtime_error("operation is undefined for an empty sketch");
    if ((rank < 0.0) || (rank > 1.0)) { throw std::invalid_argument("normalized rank cannot be less than zero or greater than 1.0"); }
    // may have a side effect of sorting level zero if needed
    setup_sorted_view();
    return sorted_view_->get_quantile(rank, inclusive);
}

template <typename T, typename C, typename A> double kll_sketch<T, C, A>::get_normalized_rank_error(bool pmf) const { return get_normalized_rank_error(min_k_, pmf); }
...
size_t kll_sketch<T, C, A>::get_serialized_size_bytes(const SerDe &) const {
    ...
    if (num_levels_ == 1 && get_num_retained() == 1) { return DATA_START_SINGLE_ITEM + sizeof(TT); }
...
size_t kll_sketch<T, C, A>::get_serialized_size_bytes(const SerDe &sd) const {
    if (is_empty()) { return EMPTY_SIZE_BYTES; }
    if (num_levels_ == 1 && get_num_retained() == 1) { return DATA_START_SINGLE_ITEM + sd.size_of_item(items_[levels_[0]]); }
    // the last integer in the levels_ array is not serialized because it can be derived
...
template <typename T, typename C, typename A> template <typename SerDe> void kll_sketch<T, C, A>::serialize(std::ostream &os, const SerDe &sd) const {
    ...
    write(os, family);
    const uint8_t flags_byte((is_empty() ? 1 << flags::IS_EMPTY : 0) | (is_level_zero_sorted_ ? 1 << flags::IS_LEVEL_ZERO_SORTED : 0) |
...
template <typename T, typename C, typename A> template <typename SerDe> auto kll_sketch<T, C, A>::serialize(unsigned header_size_bytes, const SerDe &sd) const -> vector_bytes {
    const bool is_single_item = n_ == 1;
...
kll_sketch<T, C, A> kll_sketch<T, C, A>::deserialize(std::istream &is, const SerDe &sd, const C &comparator, const A &allocator) {
    ...
    if (!is.good()) throw std::runtime_error("error reading from std::istream");
    return kll_sketch(k, min_k, n, num_levels, std::move(levels), std::move(items), capacity, std::move(min_item), std::move(max_item), is_level_zero_sorted, comparator);
}

template <typename T, typename C, typename A>
template <typename SerDe>
kll_sketch<T, C, A> kll_sketch<T, C, A>::deserialize(const void *bytes, size_t size, const SerDe &sd, const C &comparator, const A &allocator) {
 ...
 */
template <typename T, typename C, typename A> double kll_sketch<T, C, A>::get_normalized_rank_error(uint16_t k, bool pmf) {
    return pmf ? 2.446 / pow(k, 0.9433) : 2.296 / pow(k, 0.9723);
}

// for deserialization
template <typename T, typename C, typename A>
kll_sketch<T, C, A>::kll_sketch(uint16_t k, uint16_t min_k, uint64_t n, uint8_t num_levels, vector_u32 &&levels, std::unique_ptr<T, items_deleter> items, uint32_t items_size,
                                optional<T> &&min_item, optional<T> &&max_item, bool is_level_zero_sorted, const C &comparator)
    : comparator_(comparator), allocator_(levels.get_allocator()), k_(k), m_(kll_constants::DEFAULT_M), min_k_(min_k), num_levels_(num_levels),
      is_level_zero_sorted_(is_level_zero_sorted), n_(n), levels_(std::move(levels)), items_(items.release()), items_size_(items_size), min_item_(std::move(min_item)),
      max_item_(std::move(max_item)), sorted_view_(nullptr) {}

// The following code is only valid in the special case of exactly reaching capacity while updating.
// It cannot be used while merging, while reducing k, or anything else.
template <typename T, typename C, typename A> void kll_sketch<T, C, A>::compress_while_updating(void) {
    ...
    if (level == (num_levels_ - 1)) { add_empty_top_level_to_completely_full_sketch(); }
    ...
    if ((level == 0) && !is_level_zero_sorted_) { std::sort(items_ + adj_beg, items_ + adj_beg + adj_pop, comparator_); }
    if (pop_above == 0) {
...
template <typename T, typename C, typename A> uint8_t kll_sketch<T, C, A>::find_level_to_compact() const {
    uint8_t level = 0;
    while (true) {
...
template <typename T, typename C, typename A> void kll_sketch<T, C, A>::add_empty_top_level_to_completely_full_sketch() {
    ...
    if (levels_.size() < new_levels_size) { levels_.resize(new_levels_size); }
    ...
    for (uint8_t i = 0; i <= num_levels_; i++) { levels_[i] += delta_cap; }
...
template <typename T, typename C, typename A> void kll_sketch<T, C, A>::sort_level_zero() {
...
template <typename T, typename C, typename A> void kll_sketch<T, C, A>::check_sorting() const {
    ...
    for (uint8_t level = 1; level < num_levels_; ++level) {
        ...
        if (!std::is_sorted(from, to, comparator_)) { throw std::logic_error("levels must be sorted"); }
    }
}

template <typename T, typename C, typename A> quantiles_sorted_view<T, C, A> kll_sketch<T, C, A>::get_sorted_view() const {
...
template <typename T, typename C, typename A> template <typename O> void kll_sketch<T, C, A>::merge_higher_levels(O &&other, uint64_t final_n) {
...
template <typename T, typename C, typename A> void kll_sketch<T, C, A>::assert_correct_total_weight() const {
    const uint64_t total(kll_helper::sum_the_sample_weights(num_levels_, levels_.data()));
    if (total != n_) { throw std::logic_error("Total weight does not match N"); }
}

template <typename T, typename C, typename A> uint32_t kll_sketch<T, C, A>::safe_level_size(uint8_t level) const {
    if (level >= num_levels_) return 0;
    return levels_[level + 1] - levels_[level];
}

template <typename T, typename C, typename A> uint32_t kll_sketch<T, C, A>::get_num_retained_above_level_zero() const {
    if (num_levels_ == 1) return 0;
    return levels_[num_levels_] - levels_[1];
}

template <typename T, typename C, typename A> void kll_sketch<T, C, A>::check_m(uint8_t m) {
    if (m != kll_constants::DEFAULT_M) { throw std::invalid_argument("Possible corruption: M must be " + std::to_string(kll_constants::DEFAULT_M) + ": " + std::to_string(m)); }
}

template <typename T, typename C, typename A> void kll_sketch<T, C, A>::check_preamble_ints(uint8_t preamble_ints, uint8_t flags_byte) {
    const bool is_empty(flags_byte & (1 << flags::IS_EMPTY));
    ...
    if (is_empty || is_single_item) {
        if (preamble_ints != PREAMBLE_INTS_SHORT) {
            throw std::invalid_argument("Possible corruption: preamble ints must be " + std::to_string(PREAMBLE_INTS_SHORT) +
                                        " for an empty or single item sketch: " + std::to_string(preamble_ints));
        }
    } else {
        if (preamble_ints != PREAMBLE_INTS_FULL) {
            throw std::invalid_argument("Possible corruption: preamble ints must be " + std::to_string(PREAMBLE_INTS_FULL) +
                                        " for a sketch with more than one item: " + std::to_string(preamble_ints));
        }
    }
}

template <typename T, typename C, typename A> void kll_sketch<T, C, A>::check_serial_version(uint8_t serial_version) {
    if (serial_version != SERIAL_VERSION_1 && serial_version != SERIAL_VERSION_2) {
        throw std::invalid_argument("Possible corruption: serial version mismatch: expected " + std::to_string(SERIAL_VERSION_1) + " or " + std::to_string(SERIAL_VERSION_2) +
                                    ", got " + std::to_string(serial_version));
    }
}

template <typename T, typename C, typename A> void kll_sketch<T, C, A>::check_family_id(uint8_t family_id) {
    if (family_id != FAMILY) { throw std::invalid_argument("Possible corruption: family mismatch: expected " + std::to_string(FAMILY) + ", got " + std::to_string(family_id)); }
}

template <typename T, typename C, typename A> string<A> kll_sketch<T, C, A>::to_string(bool print_levels, bool print_items) const {
    ...
    if (print_items) {
        ...
        while (level < num_levels_) {
            ...
            if (from_index < to_index) { os << " level " << (unsigned int) level << ":" << std::endl; }
            for (uint32_t i = from_index; i < to_index; i++) { os << "   " << items_[i] << std::endl; }
            level++;
        }
...
template <typename T, typename C, typename A> typename kll_sketch<T, C, A>::const_iterator kll_sketch<T, C, A>::begin() const {
    return kll_sketch<T, C, A>::const_iterator(items_, levels_.data(), num_levels_);
}

template <typename T, typename C, typename A> typename kll_sketch<T, C, A>::const_iterator kll_sketch<T, C, A>::end() const {
    return kll_sketch<T, C, A>::const_iterator(nullptr, levels_.data(), num_levels_);
}

template <typename T, typename C, typename A> class kll_sketch<T, C, A>::items_deleter {
  public:
    items_deleter(uint32_t start, uint32_t num, const A &allocator) : allocator_(allocator), start_(start), num_(num) {}
    void operator()(T *ptr) {
        if (ptr != nullptr) {
...
template <typename T, typename C, typename A> void kll_sketch<T, C, A>::setup_sorted_view() const {
...
template <typename T, typename C, typename A> void kll_sketch<T, C, A>::reset_sorted_view() {
    if (sorted_view_ != nullptr) {
...
template <typename T, typename C, typename A> typename kll_sketch<T, C, A>::const_iterator &kll_sketch<T, C, A>::const_iterator::operator++() {
    ++index;
...
template <typename T, typename C, typename A> typename kll_sketch<T, C, A>::const_iterator &kll_sketch<T, C, A>::const_iterator::operator++(int) {
    const_iterator tmp(*this);
    operator++();
    return tmp;
}

template <typename T, typename C, typename A> bool kll_sketch<T, C, A>::const_iterator::operator==(const const_iterator &other) const { return index == other.index; }

template <typename T, typename C, typename A> bool kll_sketch<T, C, A>::const_iterator::operator!=(const const_iterator &other) const { return !operator==(other); }
