...
bool kll_helper::is_even(uint32_t value) { return (value & 1) == 0; }

bool kll_helper::is_odd(uint32_t value) { return (value & 1) > 0; }
...
uint32_t kll_helper::compute_total_capacity(uint16_t k, uint8_t m, uint8_t num_levels) {
    uint32_t total = 0;
    for (uint8_t h = 0; h < num_levels; h++) { total += level_capacity(k, num_levels, h, m); }
    return total;
}
...
template <typename T> void kll_helper::randomly_halve_down(T *buf, uint32_t start, uint32_t length) {
...
template <typename T> void kll_helper::randomly_halve_up(T *buf, uint32_t start, uint32_t length) {
    if (!is_even(length)) throw std::invalid_argument("length must be even");
    const uint32_t half_length = length / 2;
#ifdef KLL_VALIDATION
    const uint32_t offset = deterministic_offset();
...
template <typename T, typename C> void kll_helper::merge_sorted_arrays(T *buf, uint32_t start_a, uint32_t len_a, uint32_t start_b, uint32_t len_b, uint32_t start_c) {
 ...
 */
template <typename T, typename C>
kll_helper::compress_result kll_helper::general_compress(uint16_t k, uint8_t m, uint8_t num_levels_in, T *items, uint32_t *in_levels, uint32_t *out_levels,
    ...
    while (!done_yet) {
        ...
        if (current_level == (current_num_levels - 1)) { in_levels[current_level + 2] = in_levels[current_level + 1]; }
        ...
        if ((current_item_count < target_item_count) || (raw_pop < level_capacity(k, current_num_levels, current_level, m))) {
            ...
            if (raw_beg != out_levels[current_level]) std::move(items + raw_beg, items + raw_lim, items + out_levels[current_level]);
            ...
            if (odd_pop) {   // move one guy over
                if (out_levels[current_level] != raw_beg) items[out_levels[current_level]] = std::move(items[raw_beg]);
            ...
            if ((current_level == 0) && !is_level_zero_sorted) { std::sort(items + adj_beg, items + adj_beg + adj_pop, C()); }
...
template <typename T> void kll_helper::copy_construct(const T *src, size_t src_first, size_t src_last, T *dst, size_t dst_first) {
    while (src_first != src_last) { new (&dst[dst_first++]) T(src[src_first++]); }
}

template <typename T> void kll_helper::move_construct(T *src, size_t src_first, size_t src_last, T *dst, size_t dst_first, bool destroy) {
