...
template <typename T> struct serde<T, typename std::enable_if<std::is_arithmetic<T>::value>::type> {
    /// @copydoc serde::serialize
    void serialize(std::ostream &os, const T *items, unsigned num) const {
        bool failure = false;
        try {
            os.write(reinterpret_cast<const char *>(items), sizeof(T) * num);
        } catch (std::ostream::failure &) { failure = true; }
        if (failure || !os.good()) { throw std::runtime_error("error writing to std::ostream with " + std::to_string(num) + " items"); }
    }
    ...
    void deserialize(std::istream &is, T *items, unsigned num) const {
        ...
        } catch (std::istream::failure &) { failure = true; }
        if (failure || !is.good()) { throw std::runtime_error("error reading from std::istream with " + std::to_string(num) + " items"); }
    }
...
template <> struct serde<std::string> {
    /// @copydoc serde::serialize
    void serialize(std::ostream &os, const std::string *items, unsigned num) const {
        ...
        if (failure || !os.good()) { throw std::runtime_error("error writing to std::ostream at item " + std::to_string(i)); }
    }
    ...
    void deserialize(std::istream &is, std::string *items, unsigned num) const {
        ...
        try {
            for (; i < num; i++) {
                ...
                std::string str;
                str.reserve(length);
                for (uint32_t j = 0; j < length; j++) { str.push_back(static_cast<char>(is.get())); }
                if (!is.good()) { break; }
