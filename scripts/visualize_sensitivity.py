...
def plot_results(aggregated, output_path, show_within_variance=False):
    ...
    print(f"Found memory budgets: {memory_budgets}")
    ...
    plots: list[Plot] = [
        Plot("Update Throughput (Mops/s)", "throughput_mean", "throughput_std", title="Sensitivity Analysis (error bars = var across runs)"),
        Plot("Query Throughput (Mops/s)", "query_throughput_mean", "query_throughput_std"),
    ...
    if show_within_variance:
        plots.extend([
            Plot("ARE within-run var (avg)", "are_within_var_mean", "are_within_var_std", title="Within-run variance (error bars = var across runs)"),
            Plot("AAE within-run var (avg)", "aae_within_var_mean", "aae_within_var_std"),
        ])

    fig, axes = plt.subplots(len(memory_budgets), len(plots), figsize=(4 * len(plots), 3 * len(memory_budgets)))

    for row_idx, memory_budget in enumerate(memory_budgets):
        row_axes = axes[row_idx]
        memory_data = plot_data[memory_budget]

        for plot_idx, plot in enumerate(plots):
            ax = row_axes[plot_idx]
            if cm_data:
...
def main():
    ...
    print("\nGenerating plots...")
    plot_results(aggregated, output_path=args.output, show_within_variance=args.show_within_variance)
